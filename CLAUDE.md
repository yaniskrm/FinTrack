...
- **Un compte créé via `/signup` en local ne survit pas à un `supabase db reset`** — pas de la corruption, la base locale est aussi celle de la suite E2E Playwright et se fait réinitialiser régulièrement pendant le développement de migrations (plusieurs fois par phase, en pratique). Découvert via une confusion utilisateur légitime : après un `./scripts/dev-restart.sh` (qui, lui, ne touche jamais aux données — seul `--reset` le fait), un compte de test créé manuellement avait disparu, sans recours possible côté produit puisque le reset mot de passe n'envoie pas de vrai email en local (Inbucket seulement). **Fix : `supabase/seed.sql` recrée désormais un compte de démo aux identifiants stables (`demo@fintrack.local` / `demo1234`, recette `crypt()`/`pgcrypto` dans `auth.users` + `auth.identities`, voir le fichier) à chaque reset** — toujours disponible pour les tests manuels, jamais affecté par un reset. **Règle pour Claude : ne jamais lancer `supabase db reset` sans le signaler à l'utilisateur dans la même réponse** (même en plein milieu d'un développement de migration) — un simple rappel suffit, mais le silence est ce qui a causé la confusion ici.
