...
├── packages/
│   ├── core/                     ← Logique métier pure — ZERO dépendance React/Next/Supabase côté racine. 295 tests Vitest.
│   │   └── src/
...
│   │       ├── banking/          ← Enable Banking (Phase 13) : jwt (RS256, `node:crypto`), client (fetch API), normalize (→ `ParsedStatementRow`, réutilise le pipeline Phase 12), types — voir *point d'entrée serveur* ci-dessous
...
### Tests
- Tout nouveau code dans `packages/core` DOIT avoir un test Vitest. **295 tests actuellement, tous verts, 98,5 % de couverture.**
...
ENABLE_BANKING_APP_ID=             # Phase 13 — Control Panel Enable Banking (https://enablebanking.com/cp/applications). Server-only malgré l'absence de préfixe NEXT_PUBLIC_ : lu uniquement par des Server Actions/Route Handlers, jamais par du code exécuté navigateur
