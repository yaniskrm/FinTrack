...
Périmètre de ce déploiement (volontairement minimal, usage personnel) : base Supabase hébergée + front Vercel + SMTP Resend pour les emails d'auth. **Hors périmètre** : Edge Functions (`exchange-rates`, `send-notifications` — taux de change et notifications ne tournent pas encore en prod), secrets Vault, Enable Banking.
