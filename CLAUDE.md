...
**Phase courante : Phase 13 — Open Banking livrée (connexion bancaire via Enable Banking, consentement PSD2, synchronisation automatique réutilisant le pipeline d'import Phase 12), mergée dans `main`, CI verte. v1.0.0 + v1.1 taguées — backlog restant : v2 (voir Roadmap).**
...
**Production** : https://fintrackapp.fr (Vercel, projet `fintrack`, Root Directory `apps/web` — voir *Pièges connus* pour la configuration monorepo). Domaine acheté chez OVH, DNS géré chez OVH (`dns111.ovh.net`/`ns111.ovh.net`), DNSSEC actif — à désactiver **avant** tout futur changement de serveurs de noms (un simple changement d'enregistrement dans la zone, comme celui fait pour pointer vers Vercel, n'y touche pas). `www.fintrackapp.fr` redirige (308) vers l'apex ; l'ancien alias `fintrack-perso.vercel.app` reste actif en redirection. Base de données : projet Supabase hébergé `dgymagcuyivlujuxnolf` (Singapore), migré à jour avec `main` en Phase 13.

Emails d'authentification (confirmation d'inscription, reset de mot de passe) : SMTP personnalisé **Resend** (`smtp.resend.com:465`, expéditeur `no-reply@fintrackapp.fr`, domaine `fintrackapp.fr` vérifié côté Resend) configuré directement dans le dashboard Supabase — la clé API Resend n'a jamais transité par une session Claude Code ni par aucun fichier du repo. `mailer_autoconfirm = false` (confirmation d'email active).

⚠️ **Toute modification de ce domaine de production impose de mettre à jour, dans cet ordre** :
...
3. Domaine vérifié côté Resend (sinon le SMTP applicatif cesse d'envoyer pour ce domaine).
4. Plus tard, l'application Enable Banking **Production** (Phase 13, hors périmètre pour l'instant) — son `redirect_url` whitelisté au Control Panel doit matcher au caractère près (voir ADR-024).

**Edge Functions** : `exchange-rates` et `send-notifications` sont déployées en prod (`supabase functions deploy`, `status = ACTIVE`), `verify_jwt = true` sur les deux (voir ADR-025). Secrets Vault (noms seulement, valeurs jamais en clair) : `project_url` et `service_role_key`, lus par les deux jobs `pg_cron` (`refresh-exchange-rates` 16h UTC, `send-notifications` 08h UTC) via `net.http_post`. Secrets de fonction propres à `send-notifications` : `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` — `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` sont auto-injectés par le runtime Edge Functions, rien à créer pour ceux-là. Chemin complet vérifié manuellement (appel direct + exécution du SQL exact du job + lecture de `net._http_response`) : voir *Pièges connus*.

Périmètre de ce déploiement (volontairement minimal, usage personnel) : base Supabase hébergée + front Vercel + SMTP Resend pour les emails d'auth + les deux Edge Functions ci-dessus. **Hors périmètre** : Enable Banking (toujours en application **Sandbox**, jamais basculé en Production — voir *Open Banking* ci-dessous).
...
│   │   │   ├── (app)/            ← dashboard, transactions (+ transactions/import — Phase 12), subscriptions, budget, goals, investments, settings/{account,security,accounts,categories,notifications,export} (protégé)
│   │   │   ├── auth/callback/    ← échange du code PKCE (email + reset) ; callback/banking (Phase 13, retour de consentement Enable Banking)
...
│   │   │   ├── accounts/         ← dialog (nom/type/devise/solde initial/icône/couleur) + liste (actifs/archivés) — Phase 11 ; connect-bank-dialog + bank-connections-card (Phase 13)
...
│   │   ├── hooks/                ← use-transactions (+ catégories, remboursements), use-recurring, use-budgets, use-goals, use-investments, use-categories, use-accounts, use-import, use-banking (Phase 13) (optimistes)
│   │   ├── lib/                  ← supabase (client/server/middleware), auth (actions incl. update email/change password, mfa), transactions (+ merchant/remboursement/compte/import-actions — Phase 12), recurring, budgets, goals, investments, categories, accounts, banking (credentials/actions/queries/types — Phase 13), profile (devise par défaut), export (queries/csv/json/pdf/download), dashboard, currencies, countries (Phase 13), push, env, utils
│   │   ├── e2e/                  ← Playwright (chromium + webkit) : auth, transactions, budget/goals, export, mfa/AAL2, import (Phase 12), banking (Phase 13, dialog uniquement — round-trip non automatisable), accessibilité (axe-core)
│   │   ├── middleware.ts         ← refresh session + garde de routes + gate AAL2
│   │   └── instrumentation.ts    ← validation de config au démarrage (Enable Banking, `validateBankingConfig`)
│   └── mobile/                   ← Expo (ère mobile-first, v2 — non retravaillé ; lint en dette)
├── packages/
│   ├── core/                     ← Logique métier pure — ZERO dépendance React/Next/Supabase côté racine. 308 tests Vitest.
│   │   └── src/
...
│   │       ├── import/           ← parse-csv (RFC4180 générique), bank-statement (colonnes synonymes FR/EN, netting frais, filtre statut), duplicates (exact match) — Phase 12
│   │       ├── banking/          ← Enable Banking (Phase 13) : jwt (RS256, `node:crypto`), client (fetch API), normalize (→ `ParsedStatementRow`, réutilise le pipeline Phase 12), types, redirect-url (`buildBankingRedirectUrl`), config (`validateBankingConfig`) — voir *point d'entrée serveur* ci-dessous
│   │       ├── validators/       ← Zod (transaction/recurring/budget/goal/investment/category/account/import-schema) + impératifs (auth incl. update email/password, mfa, transaction, recurring)
│   │       └── types/            ← types partagés + SUPPORTED_CURRENCIES (165)
│   │   (+ `src/server.ts` — point d'entrée séparé `@fintrack/core/server`, voir Phase 13 ci-dessous)
...
├── supabase/
│   ├── migrations/               ← Schéma versionné (14 migrations)
│   ├── functions/
...
bank_connections    ← Consentements Open Banking par ASPSP/compte (Phase 13) — PAS de access_token/refresh_token (voir ADR-022)
```
...
**RLS activé sur les 14 tables `public`** (vérifié). Règle fondamentale :
```sql
workspace_id IN (
...
```
**Couche 2FA (AAL2)** : policies `RESTRICTIVE` sur **9 tables financières** (`categories`, `transactions`, `recurring_rules`, `budgets`, `investments`, `investment_valuations`, `goals`, `accounts`, `bank_connections`) — un user ayant un facteur TOTP vérifié ne peut lire/écrire ces données qu'avec une session `aal2` (non contournable côté serveur). Voir `20260822000000_mfa_aal2_rls.sql` + `20260825000000_investments_portfolio.sql` + `20260828000000_phase11_accounts.sql` + `20260905000000_phase13_bank_connections.sql`.

⚠️ **Règle à appliquer dès la conception pour toute future page lisant une table financière** : elle **doit** être ajoutée à `AUTH_REQUIRED_PREFIXES` et `AAL2_GATED_PREFIXES` dans `apps/web/lib/supabase/middleware.ts`, sinon un user 2FA bloqué en AAL1 atterrit sur la page et voit des données vides silencieusement au lieu d'être renvoyé vers `/mfa` (bug trouvé et corrigé en Phase 6 pour `/transactions`/`/subscriptions`, appliqué dès la conception pour `/budget`/`/goals` en Phase 7 et `/investments` en Phase 8). `/settings/accounts` (Phase 11, et `bank_connections` Phase 13 qui vit sur la même page) est déjà couverte par le préfixe générique `/settings`, déjà présent dans les deux listes — aucune modification du middleware n'a été nécessaire.
...
### Tests
- Tout nouveau code dans `packages/core` DOIT avoir un test Vitest. **308 tests actuellement, tous verts, 98,5 % de couverture.**
- Coverage cible 80 % sur `packages/core`.
- `apps/web` : E2E Playwright (`apps/web/e2e/`, chromium + webkit) — signup/login, saisie rapide, budget/objectif, les 3 formats d'export, cycle complet 2FA/AAL2 (codes TOTP réels via `otpauth`), catégories (CRUD + masquage), remboursement (marquer/régler), navigation mensuelle, mode pays (devise par défaut → pré-remplissage), comptes (CRUD + archivage, virement entre deux comptes), import de relevé CSV (parsing, exclusion de ligne, erreur colonnes non reconnues — deux des trois tests `test.skip` sur webkit, voir *Pièges connus*), Open Banking (`banking.spec.ts` — ouverture/fermeture du dialog uniquement, credential-agnostic ; le round-trip de consentement réel n'est pas automatisable, voir *Pièges connus*). Audit d'accessibilité intégré : chaque page/dialog clé est scannée par `@axe-core/playwright` (WCAG 2 A/AA) dans la même suite — c'est le mécanisme d'audit, pas une relecture manuelle. Tourne en CI (job `E2E`) contre Supabase local + un vrai build de prod.
...
### Import de fichiers bancaires (Phase 12)
- **Aucune migration de schéma** — l'import réutilise `transactions` telle quelle (Phase 3/10/11) ; `packages/core/src/import/` est un pur pipeline de transformation (fichier texte → `ParsedStatementRow[]` → `InsertRow[]`), zéro nouvelle table.
- **Parsing en deux couches** : `parse-csv.ts` (RFC 4180 générique — guillemets, échappement, CRLF/LF, BOM, détection `,` vs `;`) puis `bank-statement.ts` (sémantique bancaire par-dessus : reconnaissance de colonnes par **synonymes** FR/EN — `COLUMN_SYNONYMS`, pas des noms fixes, pour survivre à un export d'une autre banque — netting des frais contre le montant, filtre des statuts non finaux). Calibré et vérifié ligne à ligne contre un vrai relevé Revolut personnel (jamais commité, voir *Pièges connus*).
- **Statuts non finaux systématiquement exclus** (`NON_FINAL_STATUSES` : en attente/pending, renvoyé/reversed, échoué/failed/declined, annulé/cancelled…) — une opération qui n'est pas encore réglée à la banque ne doit jamais apparaître comme un fait accompli dans FinTrack.
- **Une ligne importée n'est jamais classée `transfer`** (voir ADR-020) — toujours `income`/`expense` selon le signe. Un relevé bancaire seul ne permet jamais d'établir qu'un mouvement va vers un *autre compte suivi par l'utilisateur dans FinTrack* (condition requise par `transaction_type = 'transfer'`, Phase 11) ; le supposer produirait de faux virements invisibles dans les totaux agrégés.
- **Détection de doublons strictement exacte** (`findLikelyDuplicates`) : date + type + devise + montant à ±0,01 près contre l'historique déjà en base. Délibérément aucun fuzzy matching (libellé, ±1 jour…) — un faux négatif (doublon non détecté, visible et supprimable par l'utilisateur) est préférable à un faux positif (vraie transaction masquée à tort).
- **Réutilise `suggestCategoryId`** (Phase 10, ADR-019) ligne par ligne dans la review UI — aucune logique de catégorisation dupliquée entre saisie manuelle et import.
- La conversion `amount_eur` suit exactement la même règle que la saisie manuelle (ADR-005) : taux courant au moment de l'import (pas celui de la date de la transaction historique sur le relevé), flag `rate_approximate` si le taux a plus de 48 h. Une ligne dont la devise n'a pas de taux disponible est **silencieusement exclue** du lot inséré (pas d'échec de tout l'import pour une ligne).
- La review UI (`ImportView`) permet de décocher des lignes avant import et de corriger la catégorie suggérée par ligne — rien n'est inséré tant que l'utilisateur n'a pas cliqué sur « Importer N ».

### Open Banking — Enable Banking (Phase 13)
- **Fournisseur : Enable Banking** (agrégation PSD2, https://enablebanking.com) — retenu après la fermeture de Nordigen/GoCardless aux nouvelles inscriptions (voir l'audit v2, §3) : gratuit en Restricted Production sur les comptes personnels de Yanis, sans certificat eIDAS.
- **Authentification applicative = JWT RS256 signé à la volée, jamais de token de session.** Chaque appel API (`packages/core/src/banking/jwt.ts` + `client.ts`) est signé avec la clé privée de l'application (header `kid` = App ID, `iss`/`aud` fixes, expiration 1h) — Enable Banking n'émet **aucun** `access_token`/`refresh_token` par session, contrairement à un flux OAuth générique. `session_id` scope juste les comptes autorisés par un consentement ; il ne donne aucun accès à lui seul (voir ADR-022). Conséquence : `bank_connections` n'a **aucune colonne de secret à chiffrer** — la seule donnée sensible est la clé privée applicative elle-même (`ENABLE_BANKING_PRIVATE_KEY_BASE64`, jamais en base).
- **Point d'entrée serveur séparé** : `packages/core/src/server.ts` (exporté via `"@fintrack/core/server"` dans `package.json`) réexporte `jwt.ts`/`client.ts`, qui importent `node:crypto` et font de vrais appels réseau. Le barrel racine (`"@fintrack/core"`) n'exporte que `normalize.ts`/`types.ts` (client-safe). **Ne jamais réexporter `jwt.ts`/`client.ts` depuis le barrel racine** — un Client Component qui importe `@fintrack/core` pour tout autre chose (ex. `formatCurrency`) ferait planter son bundle webpack en essayant de résoudre `node:crypto` (`UnhandledSchemeError`, trouvé au premier build de cette phase).
- **Flux de consentement** : `/settings/accounts` → « Connecter ma banque » (sélecteur pays/banque, `ConnectBankDialog`) → `startBankConnectionAction` crée une ligne `bank_connections` `status = 'pending'` (son `state` uuid sert de corrélateur) puis appelle `POST /auth` → redirection navigateur complète (`window.location.href`, pas un `fetch`) vers la banque → retour sur `/auth/callback/banking` (Route Handler, non authentifié par lui-même — repose sur le cookie de session déjà présent, même pattern que `/auth/callback` PKCE) → `POST /sessions` avec le `code` → la ligne passe `status = 'active'`, `session_id`/`enable_account_uid`/`iban`/`currency`/`valid_until` renseignés. Un consentement couvrant plusieurs comptes crée une ligne supplémentaire par compte (même `session_id`).
- **Liaison à un compte FinTrack** : une connexion `active` sans `account_id` propose de lier un compte existant ou d'en créer un (IBAN/devise pré-remplis depuis Enable Banking, solde initial à 0 — pas d'appel à `/balances` en v1, le premier `Synchroniser` peuple l'historique).
- **Synchronisation = réutilisation intégrale du pipeline d'import CSV (Phase 12)**, pas une seconde logique d'insertion : `normalizeEnableBankingTransactions` produit les mêmes `ParsedStatementRow` que `parseBankStatement`, passés par `findLikelyDuplicates` (doublons **auto-exclus**, pas de review UI comme pour l'import manuel — l'action est pensée comme un bouton « rafraîchir » à faible friction) puis `suggestCategoryId`, puis insérés via `importTransactionsAction` (Phase 12) directement — zéro code d'insertion dupliqué. `date_from` = date de dernière synchro (ou date de création de la connexion si jamais synchronisée), pagination via `continuation_key` jusqu'à épuisement.
- **Une ligne synchronisée n'est jamais classée `transfer`**, même règle et même raison que l'import CSV (ADR-020/021) — un flux de compte seul ne peut jamais prouver qu'un mouvement va vers un autre compte suivi dans FinTrack.
- **Expiration du consentement** : si `valid_until` est dépassé au moment d'une synchro, la connexion passe `status = 'expired'` et l'action échoue proprement avec un message invitant à reconnecter ; `reconnectBankConnectionAction` relance `/auth` sur la **même ligne** (nouveau `state`, aspsp identique) plutôt que d'en créer une nouvelle. Déconnecter (`disconnectBankConnectionAction`) appelle `DELETE /sessions/{id}` (best-effort) puis marque `status = 'revoked'` — jamais de suppression de ligne, même convention que comptes/catégories.
- **Couverture bancaire réelle limitée** (sandbox) : voir *Pièges connus* — seules **BBVA** et **Mock ASPSP** sont disponibles pour `country=FR` avec l'App actuelle ; aucune banque française réelle de Yanis n'est couverte tant qu'un accès production n'est pas demandé auprès d'Enable Banking.
- **Sandbox vs Production — deux applications, jamais une seule.** Le Control Panel Enable Banking n'accepte un `redirect_url` `http://localhost` que sur une application **Sandbox**, et `https` que sur une application **Production** (l'enregistrement échoue sinon avec « unsupported scheme ») — les deux ne cohabitent jamais sur une même application. `redirect_url` est construit **côté serveur** (`buildBankingRedirectUrl`, `packages/core`) à partir de `NEXT_PUBLIC_SITE_URL`, jamais de l'origine live du navigateur — garantit une correspondance caractère-pour-caractère avec l'URL enregistrée, quel que soit l'alias de domaine utilisé pour accéder à l'app. Bascule Sandbox/Production = **mêmes noms de variables** (`ENABLE_BANKING_APP_ID`/`ENABLE_BANKING_PRIVATE_KEY_BASE64`/`NEXT_PUBLIC_SITE_URL`), valeurs différentes par environnement (Sandbox + `http://localhost:3000` dans `.env.local`, Production + le vrai domaine `https` dans les variables Vercel) — zéro branchement de code.
- **Validation de config au démarrage** (`validateBankingConfig`, `packages/core`, appelée depuis `apps/web/instrumentation.ts`) échoue explicitement si `ENABLE_BANKING_APP_ID`/`ENABLE_BANKING_PRIVATE_KEY_BASE64` ne sont définies qu'à moitié, ou si `NEXT_PUBLIC_SITE_URL` n'est pas `https` en vraie production Vercel — plutôt qu'un `REDIRECT_URI_NOT_ALLOWED` cryptique au moment du callback. Signal de « vraie production » = `VERCEL_ENV === "production"`, **surtout pas** `NODE_ENV === "production"` — voir *Pièges connus*.
...
NEXT_PUBLIC_SITE_URL=              # fallback redirect email + base du redirect_url Enable Banking (load-bearing là, pas un fallback — doit matcher au caractère près l'URL enregistrée au Control Panel). http://localhost:3000 en local, le vrai domaine https en prod Vercel
ENABLE_BANKING_APP_ID=             # Phase 13 — Control Panel Enable Banking (https://enablebanking.com/cp/applications). Server-only malgré l'absence de préfixe NEXT_PUBLIC_ : lu uniquement par des Server Actions/Route Handlers, jamais par du code exécuté navigateur. Sandbox en local, Production sur Vercel — deux applications distinctes, mêmes noms de variables (voir Open Banking ci-dessus et Pièges connus)
ENABLE_BANKING_PRIVATE_KEY_BASE64= # Phase 13 — base64 du fichier .pem téléchargé au moment de l'enregistrement de l'app (nom de fichier = App ID) ; base64 choisi pour éviter les sauts de ligne littéraux dans une valeur .env — voir CLAUDE.md, Pièges connus
...
# NB : ENABLE_BANKING_APP_ID/ENABLE_BANKING_PRIVATE_KEY_BASE64 ne sont PAS configurés en secrets CI (délibéré,
# décision de sécurité de Yanis — stocker une clé d'API financière réelle en CI est un choix qui lui appartient).
# Sans eux, le bouton « Connecter ma banque » échoue proprement (message d'erreur géré, testé en E2E).
```
...
15. **Import de fichiers bancaires** — parsing CSV (colonnes synonymes FR/EN), détection de doublons, suggestion de catégorie réutilisée, rattachement à un compte ✅
16. **Open Banking** — connexion bancaire via Enable Banking (consentement PSD2), liaison/création de compte, synchronisation automatique réutilisant le pipeline d'import ✅
17. **Module IA** — v2
...
- **Phase 12** ✅ Import de fichiers bancaires — parsing CSV RFC4180 générique + reconnaissance de colonnes par synonymes FR/EN, netting des frais, filtre des statuts non finaux, détection de doublons exacte, réutilisation de `suggestCategoryId` (Phase 10) et rattachement à un compte (Phase 11) dans une review UI avant insertion. **Aucune migration de schéma** (réutilise `transactions` tel quel). Calibré contre un vrai relevé bancaire personnel (jamais commité — voir *Pièges connus*).
- **Phase 13** ✅ Open Banking (Enable Banking) — connexion bancaire par consentement PSD2 (`/settings/accounts` → « Connecter ma banque »), table `bank_connections` (RLS + AAL2, sans token de session — voir ADR-022), liaison à un compte existant ou création automatique (IBAN/devise), synchronisation manuelle réutilisant intégralement le pipeline de dédoublonnage/catégorisation de l'import CSV (Phase 12). Couverture bancaire réelle limitée par le palier sandbox actuel de l'app (BBVA + Mock ASPSP seulement pour la France) — voir *Pièges connus*.

Prochaine étape : backlog v2 (voir ci-dessous) — pas de Phase 14 planifiée pour l'instant.

**Backlog v2** : app mobile Expo (reprise), module IA, multi-utilisateurs, notifications intelligentes (détection d'anomalies de dépenses).
...
- **ADR-021** *(Phase 12)* : une ligne importée n'est **jamais** classée `'transfer'`, même si le libellé du relevé suggère un virement — toujours `income`/`expense` selon le signe (cohérent avec ADR-020 : `'transfer'` exige un `to_account_id` vers un *autre compte suivi dans FinTrack*, qu'un relevé bancaire seul ne peut jamais établir avec certitude ; le deviner produirait de faux virements invisibles dans `calculateTotals`). Détection de doublons volontairement **exact-match uniquement** (date + type + devise + montant ±0,01) plutôt qu'un score de similarité sur le libellé — alternative rejetée pour ne jamais risquer de masquer une transaction réelle distincte (faux positif) au profit d'un doublon occasionnel non détecté mais visible et supprimable (faux négatif), jugé le compromis le plus sûr pour des données financières. Reconnaissance de colonnes par **synonymes** (`COLUMN_SYNONYMS`, FR/EN) plutôt que noms de colonnes fixes — calibré au départ sur un relevé Revolut réel (colonnes en français), pensé dès le départ pour survivre à l'export d'une autre banque sans réécriture.
- **ADR-022** *(Phase 13)* : `bank_connections` **sans colonnes `access_token`/`refresh_token`**, contrairement au schéma générique initialement envisagé — Enable Banking n'émet aucun token de session : chaque appel API est authentifié par un JWT RS256 signé à la volée avec la clé privée applicative (jamais stockée en base), `session_id` scopant juste les comptes autorisés sans donner accès à lui seul. Conséquence directe : aucun secret par connexion à chiffrer, la seule donnée sensible du dispositif est la clé privée applicative elle-même (env var serveur, jamais en base). `jwt.ts`/`client.ts` (qui importent `node:crypto` et font de vrais appels réseau) exposés via un point d'entrée `"@fintrack/core/server"` séparé du barrel racine — un Client Component qui importe `@fintrack/core` pour tout autre chose ne doit jamais risquer de faire planter son bundle webpack sur `node:crypto` (`UnhandledSchemeError`, rencontré au premier build de cette phase).
- **ADR-023** *(Phase 13)* : la synchronisation Open Banking **réutilise intégralement le pipeline de l'import CSV** (Phase 12) plutôt que d'implémenter une seconde logique d'insertion — `normalizeEnableBankingTransactions` produit le même `ParsedStatementRow` que `parseBankStatement`, passé par les mêmes `findLikelyDuplicates`/`suggestCategoryId`/`importTransactionsAction`. Alternative rejetée : un chemin d'insertion dédié à Open Banking — aurait dupliqué toute la logique de dédoublonnage/catégorisation/conversion de devise sans bénéfice, les deux sources produisant fondamentalement le même besoin (transactions bancaires à normaliser, dédupliquer, catégoriser, convertir). Contrairement à l'import manuel (review UI, doublons décochables), la synchro auto-exclut les doublons sans review — assumé comme un bouton « rafraîchir » à faible friction, pas un import ponctuel supervisé.
- **ADR-024** *(fix post-Phase 13)* : `redirect_url` Enable Banking construit **côté serveur** à partir de `NEXT_PUBLIC_SITE_URL` (`buildBankingRedirectUrl`), jamais de l'origine live du navigateur (`window.location.origin`, l'approche initiale de la Phase 13). Déclencheur : l'enregistrement d'une application **Production** chez Enable Banking a échoué (`http://localhost:3000` rejeté — une application Production n'accepte que `https`), révélant que Sandbox et Production exigent deux applications distinctes avec des `redirect_url` différents. Alternative rejetée : introduire une nouvelle variable (`NEXT_PUBLIC_APP_URL`) — `NEXT_PUBLIC_SITE_URL` existait déjà pour exactement ce rôle (URL de base canonique pour des redirects absolus, déjà utilisée pour les emails auth). Bascule Sandbox/Production = mêmes noms de variables (`ENABLE_BANKING_APP_ID`/`ENABLE_BANKING_PRIVATE_KEY_BASE64`/`NEXT_PUBLIC_SITE_URL`), valeurs différentes par environnement — aucun branchement de code, une pure question de configuration par environnement de déploiement. `validateBankingConfig` échoue explicitement au démarrage (`instrumentation.ts`) sur une config incohérente, avec `VERCEL_ENV === "production"` comme signal de « vraie prod » plutôt que `NODE_ENV` (voir *Pièges connus*).
- **ADR-025** *(mise en prod des Edge Functions)* : le secret Vault `service_role_key` (lu par les jobs `pg_cron` `refresh-exchange-rates`/`send-notifications` pour appeler leur Edge Function via `net.http_post`, header `Authorization: Bearer`) reste la **clé JWT legacy `service_role`**, pas la nouvelle clé `sb_secret_...`. Les deux fonctions gardent **`verify_jwt = true`** (exigence de sécurité : elles écrivent en base avec les droits `service_role`, donc ne doivent pas être appelables sans authentification). Vérifié dans la doc Supabase (*Migrating to publishable and secret API keys*) avant tout choix : le contrôle `verify_jwt` de la passerelle essaie de parser tout `Authorization: Bearer` comme un JWT — une clé `sb_secret_...` (qui n'est pas un JWT) y échoue explicitement avec `Invalid JWT` ; le pattern documenté par Supabase pour les nouvelles clés côté service-à-service (`pg_net`, Database Webhooks) est de désactiver `verify_jwt` et d'envoyer la clé sur le header `apikey` avec `auth: 'secret'` dans le code de la fonction. Désactiver `verify_jwt` étant explicitement exclu ici, la clé JWT legacy reste la seule option compatible avec le code existant (`Authorization: Bearer`, déjà écrit ainsi dans les deux migrations de planification) — zéro changement de code, uniquement la valeur du secret Vault.
...
- **Extraire les données d'un fichier Apple Numbers (`.numbers`) nécessite un outillage dédié** (Phase 12 — un relevé bancaire réel avait été déposé au format `.numbers`) : c'est une archive zip contenant des données binaires IWA/protobuf, illisible telle quelle. La librairie Python `numbers-parser` (installée dans un venv jetable, jamais committé) permet d'en extraire les lignes du tableau. À refaire de la même façon si un futur relevé arrive dans ce format plutôt qu'en CSV natif.
- **Donnée bancaire réelle déposée dans le repo pour calibrer un parser (Phase 12) : jamais commise, jamais laissée traîner.** `.gitignore` étendu (`*.numbers`, `releve_*.csv`) avant même d'ouvrir le fichier ; tout export CSV intermédiaire dérivé (scratchpad) et le venv Python jetable ont été supprimés une fois le parseur calibré et ses tests écrits contre des fixtures **synthétiques** (jamais les vraies valeurs). Règle à reconduire pour toute donnée personnelle réelle qu'un futur travail similaire nécessiterait d'inspecter.
- **Flake webkit non résolu, scopé en connaissance de cause (Phase 12) : après un insert via Server Action, la lecture Server Component suivante sur `/transactions` peut rendre 0 lignes si la navigation programmée Playwright suit l'insert quasi instantanément — uniquement sous WebKit, jamais reproduit sous Chromium.** La donnée elle-même est prouvée correcte et durable (vérifiée directement via `psql`, présente à l'instant même où le toast de succès s'affiche) — le problème est isolé à un artefact de timing propre au moteur de rendu WebKit sous Playwright, pas à un bug applicatif. Pistes explorées et écartées : cache HTTP (la réponse envoie déjà `cache-control: no-store`), Data Cache de `fetch` côté Next (forcer `no-store` sur chaque appel supabase-js n'a rien changé), attendre `networkidle` après le `goto` (aucune différence — le rendu est stable, juste faux), une boucle de re-tentative bornée (5 reloads / 2 s+, toujours en échec dans ce test précis malgré une répro quasi identique fiable ailleurs avec bien moins de délai). Scopé via `test.skip(browserName === "webkit", …)` sur les deux tests E2E concernés (`apps/web/e2e/import.spec.ts`), même traitement que la limitation Push API déjà documentée ci-dessus — à ne pas re-tenter de corriger sans un signal nouveau (ex. si Playwright ou WebKit publient un correctif connu pour ce genre de timing).
- **Une date de fixture E2E codée en dur casse silencieusement dès que le temps réel passe le mois suivant** (trouvé en validant la Phase 13, sur un test de la Phase 12) : `/transactions` filtre par défaut sur le mois **courant** (`monthKey(new Date().toISOString())`, `transactions-view.tsx`) — `import.spec.ts` utilisait des dates d'août 2026 codées en dur, devenues invisibles par défaut une fois le 1er septembre passé. Symptôme trompeur : ressemblait exactement au flake webkit ci-dessus (0 ligne visible après import), reproduit de façon déterministe sur chromium **et** webkit une fois le mois changé — a bien failli être diagnostiqué à tort comme une régression de Phase 13 avant qu'un test contre `main` non modifié ne confirme que ce n'était pas lié au nouveau code. **Toujours générer les dates de fixture relativement à `new Date()` dans le test**, jamais une date absolue, dès qu'un test dépend d'une vue filtrée par mois/période courante.
- **`packages/core` doit déclarer explicitement `@types/node` s'il utilise `node:crypto`/`Buffer`/`fetch`/`URLSearchParams`** (Phase 13) : fonctionnait en local (un autre paquet du monorepo, `apps/web`, le déclare déjà, et pnpm avait déjà résolu une version quelque part dans l'arbre) mais cassait `tsc` en CI avec un `pnpm install --frozen-lockfile` propre (`Cannot find name 'fetch'`, `Cannot find module 'node:crypto'`) — un paquet ne doit jamais compter sur une dépendance résolue par un autre paquet du monorepo. Corrigé en ajoutant `@types/node` en devDependency de `packages/core`, ce qui a aussi éliminé une résolution flottante non épinglée (`@types/node@25.5.0` picked up par défaut) au profit de la version explicitement alignée sur `apps/web` (`^22.10.2`).
- **Un Client Component qui importe `@fintrack/core` pour une fonction pure (ex. `formatCurrency`) fait planter tout le bundle webpack si le barrel racine réexporte, même transitivement, un module qui importe `node:crypto`** (Phase 13, `UnhandledSchemeError` au premier build après l'ajout du client Enable Banking) — webpack tente de résoudre l'import Node au niveau statique du graphe de modules, indépendamment de si le Client Component appelle réellement la fonction concernée. Fix structurel : tout code serveur-only (signature JWT, appels réseau) vit dans `packages/core/src/server.ts`, exporté via un point d'entrée séparé `"@fintrack/core/server"` (nouveau champ `exports` dans `package.json`), jamais réexporté depuis `index.ts`. Voir ADR-022.
- **Couverture bancaire réelle très restreinte sur le palier Enable Banking actuel** (Phase 13, vérifié en conditions réelles avec les identifiants de Yanis) : `GET /aspsps?country=FR` ne renvoie que **BBVA** et **Mock ASPSP** — aucune des banques françaises réelles pressenties (Revolut, BNP, Société Générale, Crédit Agricole…) n'est accessible tant qu'un accès production n'a pas été demandé auprès d'Enable Banking (l'app est en sandbox/test). Confirmé aussi que `redirect_url` doit être **whitelisté au préalable dans le Control Panel** (`https://enablebanking.com/cp/applications`) — un `POST /auth` avec une URL non enregistrée échoue en `400 REDIRECT_URI_NOT_ALLOWED` ; `http://localhost:3000/auth/callback/banking` l'était déjà.
- **Même la « Mock ASPSP » de test d'Enable Banking ne s'authentifie pas avec des identifiants sandbox jetables — elle redirige vers une vraie page de connexion `enablebanking.com/sign-in`, le compte développeur du Control Panel lui-même.** Vérifié en pilotant un vrai navigateur Playwright à travers le flux de consentement jusqu'à ce point (JWT signé, `POST /auth` réel, redirection réelle vers la banque). Conséquence : le round-trip complet de consentement **n'est pas automatisable en E2E/CI** sans stocker un identifiant de connexion personnel (email/mot de passe du dashboard Enable Banking), ce qui serait disproportionné pour un test automatisé — même traitement que la limitation Push API (v1.1). Les tests E2E de `banking.spec.ts` se limitent donc à l'ouverture/fermeture du dialog et à son rendu, jamais au flux de redirection réel.
- **Le toast d'erreur sonner (`richColors`, thème clair) a un vrai échec de contraste WCAG AA sur son propre texte par défaut** (Phase 13, trouvé par le scan axe du dialog « Connecter ma banque » — le premier dialog scanné où un toast d'erreur s'affiche systématiquement, faute d'identifiants Enable Banking en CI) : `--error-text` par défaut de sonner (`hsl(360 100% 45%)`, ≈ `#e60000`) n'atteint que 4,34:1 contre son propre fond d'erreur, sous le seuil 4,5:1. Corrigé par une surcharge CSS (`[data-sonner-toaster] { --error-text: hsl(360 100% 30%) !important; }`, `!important` nécessaire car sonner fixe la variable à la même spécificité) — même principe que le fix `--success` (ADR-017), mais sur un token tiers plutôt qu'un token du design system. **Piège distinct rencontré en le corrigeant** : le scan axe restait flaky (~25 % d'échecs même avec un `waitForTimeout(500)` fixe) car il pouvait capturer le toast **en cours d'animation d'entrée**, dont les couleurs intermédiaires interpolées sont temporairement bien plus contrastées-faibles que l'état final (jamais la même paire de couleurs deux fois de suite dans les échecs observés — signature d'une transition, pas d'un bug figé). Fix : attendre la fin réelle des Web Animations du toast (`element.getAnimations({ subtree: true })` + `Promise.all(...finished)`) plutôt qu'un délai fixe, avant de lancer le scan axe.
- **Une application Enable Banking Production n'accepte qu'un `redirect_url` `https`, une application Sandbox qu'un `redirect_url` `http://localhost` — les deux ne peuvent jamais cohabiter sur la même application** (fix post-Phase 13) : enregistrer une app Production avec `http://localhost:3000/auth/callback/banking` échoue explicitement (« unsupported scheme ») au moment même de l'enregistrement, avant tout code. Il faut deux applications Enable Banking distinctes (Sandbox pour le dev local, Production pour Vercel), avec des identifiants différents mais les **mêmes noms de variables** — voir ADR-024 et la section Open Banking ci-dessus.
- **Le signal de « vraie production » pour une validation de config doit être `VERCEL_ENV`, pas `NODE_ENV`** (fix post-Phase 13, reproduit puis corrigé avant de committer) : `next start` positionne `NODE_ENV=production` pour **tout** build optimisé, y compris la validation locale que ce repo fait systématiquement avant chaque passe E2E (`NEXT_DIST_DIR=.next-verify`, voir *Workflow Git*). Une validation gatée sur `NODE_ENV === "production"` (`instrumentation.ts`, `validateBankingConfig`) faisait donc planter le démarrage du serveur E2E **local**, avec des identifiants Sandbox réels configurés et sans `NEXT_PUBLIC_SITE_URL` définie (le cas de tous les jours). `VERCEL_ENV` n'est positionnée que par la plateforme Vercel elle-même pour un vrai déploiement — jamais en local, jamais en CI GitHub Actions — c'est le signal correct pour distinguer « build optimisé » de « déployé en production ».
- **La tâche `lint` de `turbo.json` n'avait pas `dependsOn: ["^build"]`, contrairement à `build`/`typecheck`/`test`** (fix post-Phase 13, trouvé en vérifiant le fix Enable Banking en CI) : `@fintrack/web:lint` (ESLint typé, qui construit son propre programme TypeScript) pouvait donc s'exécuter avant, ou en parallèle de, `@fintrack/core:build` — sans `packages/core/dist` à jour, la résolution de types de `@fintrack/core` s'effondre et des centaines d'erreurs `no-unsafe-assignment`/`no-unsafe-member-access` cascadent sur tous les fichiers qui importent depuis ce paquet, même totalement sans rapport avec le changement en cours. Reproduit de façon déterministe en local (`rm -rf packages/core/dist && pnpm turbo lint --filter=@fintrack/web` → 1040 erreurs) et confirmé non aléatoire en CI (échec identique sur un rerun). Fix : ajouter `dependsOn: ["^build"]` à la tâche `lint`, comme les trois autres.
- **`resolveSiteOrigin` (`apps/web/lib/env.ts`) préférait l'`Origin` du navigateur à `NEXT_PUBLIC_SITE_URL`** (fix post-Phase 13, trouvé en migrant vers `fintrackapp.fr`) : inoffensif tant qu'un seul domaine servait l'app (l'`Origin` d'une requête légitime vaut de toute façon l'URL de prod), mais incohérent avec le principe déjà appliqué à `buildBankingRedirectUrl`/ADR-024 (ne jamais construire une URL qui doit matcher un allow-list externe à partir d'une donnée fournie par la requête). Inversé : l'env var est prioritaire, l'`Origin` ne sert plus que de repli si elle est absente. `signUpAction`/`requestPasswordResetAction`/`updateEmailAction` en bénéficient sans changement de leur côté.
- **`updatePasswordAction` redirigeait vers `/dashboard` sans vérifier l'AAL, contrairement à `signInAction`** (fix post-Phase 13/Phase 6, trouvé en auditant le flux de reset de mot de passe avant la mise en prod des emails réels) : un utilisateur TOTP qui termine un reset de mot de passe est réellement protégé côté données (le middleware revérifie l'AAL en direct sur chaque requête vers `/dashboard`, RLS AAL2 en backstop) — mais sans la même correction que `signInAction` (voir *pièges connus* Phase 6 : une `redirect()` de Server Action ne chaîne pas fiablement à travers celle du middleware, l'URL du navigateur peut rester bloquée sur `/dashboard` même si le contenu servi est bien `/mfa`), l'expérience utilisateur aurait été cassée au même endroit exact. Fix : `updatePasswordAction` calcule maintenant lui-même la destination (`getAuthenticatorAssuranceLevel()`), comme `signInAction`.
- **Le SMTP applicatif (Resend) a été configuré directement dans le dashboard Supabase, jamais via une session Claude Code** (fix post-Phase 13, migration `fintrackapp.fr`) — décision délibérée de Yanis pour que la clé API Resend ne transite jamais par un terminal, un fichier ou un log, même temporairement. Vérification faite uniquement en lecture via la Management API (`GET /v1/projects/{ref}/config/auth`), jamais en écriture sur les champs `smtp_*`. `smtp_max_frequency` (cooldown minimal entre deux emails vers une même adresse) passe à 60 s avec un SMTP personnalisé — le débit global n'est ensuite plus borné par la limite très basse du mailer intégré de Supabase, mais par le plan Resend.
- **Les deux jobs `pg_cron` (`refresh-exchange-rates`, `send-notifications`) tournaient en silence sans jamais rien faire depuis leur création (Phase 6/v1.1), sans la moindre erreur visible.** Cause : les secrets Vault `project_url`/`service_role_key` qu'ils attendent (voir migrations `20260823000001`/`20260826000000`) n'avaient jamais été créés sur la base hébergée, **et** aucune des deux Edge Functions n'avait jamais été déployée en prod (`supabase functions list` → vide). La garde SQL du job (`where exists (select 1 from vault.decrypted_secrets where name = 'project_url')`) rend ce no-op silencieux **par construction** : `cron.job_run_details.status = succeeded`, `return_message = "0 rows"` — un statut « réussi » qui ne prouve absolument pas qu'un appel HTTP a eu lieu. Le seul signal qui aurait pu alerter : la fraîcheur de `exchange_rates.updated_at`, jamais rafraîchie depuis le seed de migration. **Leçon pour tout futur job `pg_cron`+`pg_net`+Vault de ce type : un statut `succeeded` de `cron.job_run_details` ne garantit rien sur l'appel réseau lui-même — toujours vérifier séparément `net._http_response` (`status_code`/`content`) et un effet de bord observable en base (ici la fraîcheur des données), jamais l'un sans l'autre.** Fix : fonctions déployées, secrets Vault créés (voir ADR-025), chemin complet revérifié manuellement (appel direct + SQL exact du job + lecture de `net._http_response`).
- **`cron.job_run_details` ne conserve qu'une fenêtre très courte sur cette base hébergée** (~13h observées, 2 lignes au total au moment de l'audit malgré des jobs actifs depuis plus d'un mois) — Supabase purge cette table automatiquement, ce n'est pas une anomalie. Ne pas s'appuyer dessus pour un historique au-delà de la dernière journée ; `net._http_response` (via `pg_net`) est la source à interroger pour le détail d'un appel précis, juste après l'avoir déclenché.
- **Connexion `psql` à la base hébergée : le pooler transaction utilise `aws-1-ap-southeast-1.pooler.supabase.com:6543`, pas `aws-0-...:5432`** (l'hôte `aws-0`/port 5432 — pattern historique documenté ailleurs — renvoie `tenant/user not found` pour ce projet). La connexion directe (`db.<ref>.supabase.co:5432`) échoue aussi dans cet environnement (résolution IPv6 uniquement, `No route to host` sans IPv6 sortant). Toujours vérifier l'hôte/port exacts via `GET /v1/projects/{ref}/config/database/pooler` (Management API) plutôt que de deviner le pattern `aws-0`/5432.
- **`GET /v1/projects/{ref}/api-keys?reveal=false` ne masque PAS les clés JWT legacy (`anon`/`service_role`), seulement la nouvelle clé `sb_secret_...`.** Découvert en listant les clés pour déterminer le bon format à utiliser dans un secret Vault (voir ADR-025) : les deux clés legacy sont revenues en clair dans la sortie malgré `reveal=false`, alors que la clé `sb_secret_` était bien tronquée. À traiter comme si `reveal` n'existait pas pour les clés legacy — ne jamais passer cet appel brut sans filtrer/rediriger la sortie avant qu'elle n'atteigne un terminal ou un log, exactement comme pour toute création de secret Vault.
